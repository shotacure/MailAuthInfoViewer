...
* **Trusted Authentication Filtering (authserv-id):** Filters `Authentication-Results` headers to only trust those from the receiving mail server. When the most recent `Received` hops are internal handoffs whose `by` is an IP literal (e.g. Gmail / Google Workspace appending `Received: by 2002:<IPv6>` at the top), they are skipped so the boundary MTA's hostname (e.g. `mx.google.com`) is matched against the `authserv-id` instead of the IP literal — otherwise the legitimate `Authentication-Results` would be discarded and DKIM/DMARC would wrongly show as `none`.
    * **信頼できる認証結果のフィルタリング (authserv-id):** 受信メールサーバーの `Authentication-Results` ヘッダのみを信頼します。最新の `Received` ホップが、`by` が IP リテラルの内部ハンドオフ（例: Gmail / Google Workspace が最上段に積む `Received: by 2002:<IPv6>`）の場合はそれを読み飛ばし、境界 MTA のホスト名（例: `mx.google.com`）を `authserv-id` と照合します。IP リテラルをそのまま使うと正規の `Authentication-Results` が破棄され、DKIM/DMARC が誤って `none` 表示になってしまうためです。
