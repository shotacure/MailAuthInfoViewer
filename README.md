...
* **Trusted Authentication Filtering (authserv-id):** Filters `Authentication-Results` headers to only trust those from the receiving mail server. The trusted host is located by finding the boundary MTA — the most recent `Received` hop that accepted the message from a public IP address — so that internal handoffs added *after* authentication are skipped. This correctly handles Gmail / Google Workspace (which prepend `Received: by 2002:<IPv6>` as an IP-literal internal hop) and providers such as Fastmail (where the final delivery hop, e.g. a Cyrus LMTP host like `slotpiXXmYY`, differs from the authenticating host such as `phl-mx-NN.messagingengine.com`, with several `*.internal` hops in between). When no public-facing hop is present (e.g. self-sent mail routed entirely inside the provider), the first real public hostname in the chain is used instead. Without this, the legitimate `Authentication-Results` would be matched against the wrong host, discarded, and SPF/DKIM/DMARC would wrongly show as `none`.
    * **信頼できる認証結果のフィルタリング (authserv-id):** 受信メールサーバーの `Authentication-Results` ヘッダのみを信頼します。信頼するホストは「境界 MTA」——公開 IP アドレスからメールを受け取った最新の `Received` ホップ——を特定して決定し、認証の*後*に付与される内部ハンドオフを読み飛ばします。これにより Gmail / Google Workspace（IP リテラルの内部ホップ `Received: by 2002:<IPv6>` を最上段に積む）や、Fastmail のように最終配送ホップ（例: Cyrus LMTP の `slotpiXXmYY`）が認証ホスト（例: `phl-mx-NN.messagingengine.com`）と異なり間に複数の `*.internal` ホップを挟む構成にも正しく対応します。公開ホップが存在しない場合（例: プロバイダ内で完結する自己送信メール）は、経路中の最初の実ホスト名を用います。これを行わないと、正規の `Authentication-Results` が誤ったホストと照合されて破棄され、SPF/DKIM/DMARC が誤って `none` 表示になってしまいます。
* **Reported-but-unverified results notice:** When a method's trusted verdict is `none` but a result is present only in an untrusted source — a standard `Authentication-Results` header whose `authserv-id` could not be tied to your receiving server, or the `ARC-Authentication-Results` chain — the value is surfaced as a compact, dotted-underlined `ℹ️ Reported, unverified` pill in the header (full caveat on hover). This avoids misleading bare `none` for legitimate mail whose receiving infrastructure records results in a non-standard place, while keeping the conservative principle intact: these reported values are clearly marked as not independently verified and **do not affect the badge, the cards, or the verdict** (ARC results in particular are forgeable and not cryptographically checked here).
    * **報告値（未検証）の通知:** ある方式の信頼済み判定が `none` でも、信頼できないソース——`authserv-id` を受信サーバーに帰属できなかった標準 `Authentication-Results` や `ARC-Authentication-Results` チェーン——にのみ結果がある場合、その値をヘッダ内に `ℹ️ 報告・未検証` のコンパクトなピル（点線下線・hover で全文の断り書き）として控えめに提示します。受信側インフラが非標準の場所に結果を記録した正規メールを、いきなり `none` だけで誤解させないための表示です。保守原則は維持し、これらの報告値は「独立検証していない」と明示したうえで、**バッジ・各カード・総合判定には一切影響させません**（特に ARC は偽造可能で本アドオンでは暗号検証していません）。
